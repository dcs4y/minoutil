...
func init() {
	emailClient = gomail.NewDialer(common.WS.EmailConfig.Host, common.WS.EmailConfig.Port, common.WS.EmailConfig.Username, common.WS.EmailConfig.Password)
	tpl, err := template.ParseGlob(filepath.Join(common.ResourcesPath, "email/*.html"))
	if err != nil {
		log.Println(err)
	}
	emailTemplate = tpl
...
func SendSampleEmail(to, title, content string) error {
	email := &Email{
		From:    common.WS.EmailConfig.Username,
		To:      []string{to},
		Subject: title,
		Body:    content,
	}
...
func SendTemplateEmail(to, title, templateName string, params interface{}) error {
	b := bytes.NewBuffer([]byte{})
	emailTemplate.ExecuteTemplate(b, templateName, params)
	return SendSampleEmail(to, title, b.String())
}
