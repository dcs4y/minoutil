...
func init() {
	Client = NewClient(common.WS.RedisConfig)
	log = logutil.Log.WithField("kind", "redis")
}

func NewClient(redisConfig *common.RedisModel) *client {
	// 初始化Redis连接池
	redisPool := &redis.Pool{ //实例化一个连接池
		...
		Dial: func() (redis.Conn, error) { //要连接的redis数据库
			...
			setDB := redis.DialDatabase(redisConfig.Database)
			options[0] = setDB
			if redisConfig.Password != "" {
				setPassword := redis.DialPassword(redisConfig.Password)
				options = append(options, setPassword)
			}
			return redis.Dial("tcp", redisConfig.Host+":"+strconv.Itoa(redisConfig.Port), options...)
		},
	}
	return &client{pool: redisPool}
}

// ClosePool 关闭连接池，慎用。
func (rc *client) ClosePool() {
	rc.pool.Close() //关闭连接池
}
...
// SetObject 设置key的值
func (rc *client) SetObject(key string, value interface{}, second int) error {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	if second > 0 {
...
func (rc *client) SetExpire(key string, second int) error {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	if second > 0 {
...
func (rc *client) GetKeys(pattern string) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
// Delete 删除KEY
func (rc *client) Delete(key string) (int, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetString(key string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetInt(key string) (int, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetInt64(key string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetFloat64(key string) (float64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetBool(key string) (bool, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetByte(key string) ([]byte, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GetObject(key string) (interface{}, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) Lock(key string, value interface{}) (bool, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
// SetObjects 设置多key的值
func (rc *client) SetObjects(kv ...interface{}) error {
	if len(kv)%2 != 0 {
...
// GetStrings 获取多key的值
func (rc *client) GetStrings(key ...interface{}) (map[interface{}]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
// HashSet 设置hash的值
func (rc *client) HashSet(redisKey string, hashKey string, value interface{}) error {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) HashSetAll(redisKey string, hashKeyValue ...interface{}) error {
...
func (rc *client) HashGetString(redisKey string, hashKey string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) HashGetAll(redisKey string) (map[string]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) HashDelete(redisKey string, hashKey ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) HashLock(redisKey string, hashKey string, value interface{}) (bool, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListLeftPush(key string, value ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListRightPush(key string, value ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	value = append([]string{key}, value...)
...
func (rc *client) ListAddBefore(key string, pivot string, value string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListAddAfter(key string, pivot string, value string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListUpdate(key string, index int64, value string) error {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListGetString(key string, index int64) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListGetRange(redisKey string, start, end int64) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListGetAll(redisKey string) ([]string, error) {
	return rc.ListGetRange(redisKey, 0, -1)
}

// ListDelete 删除列表中的value
func (rc *client) ListDelete(key string, value string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListLeftPopString(key string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListRightPopString(key string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ListCut(key string, start int64, end int64) error {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
// ListLength 获取列表长度
func (rc *client) ListLength(key string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetAdd(key string, value ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	value = append([]string{key}, value...)
...
func (rc *client) SetLength(key string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetGetAll(key string) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetInter(keys ...string) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetUnion(keys ...string) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetDeleteRand(key string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) SetDelete(key string, value ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	value = append([]string{key}, value...)
...
func (rc *client) SetIsMember(key string, value string) (bool, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ZSetAdd(key string, scoreValue ...interface{}) (int64, error) {
...
func (rc *client) ZSetDelete(key string, value ...string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	value = append([]string{key}, value...)
...
func (rc *client) ZSetDeleteByScore(key string, minScore, maxScore float64) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ZSetLength(key string) (int64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ZSetGetScore(key string, value string) (float64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) ZSetGetByScore(key string, minScore, maxScore float64, descending bool) ([]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	if descending {
...
func (rc *client) ZSetGetByScoreWithScore(key string, minScore, maxScore float64, descending bool) (map[string]string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
	if descending {
...
func (rc *client) ChannelSubscribe(channel ...string) chan redisMessage {
...
func (rc *client) ChannelPublish(channel string, message interface{}) (int, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
// LuaExecute 执行lua脚本
func (rc *client) LuaExecute(luaScript string, redisKeys []string, luaParams []string) (interface{}, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) LuaExecuteWithSha1(sha1 string, redisKeys []string, luaParams []string) (interface{}, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) LuaLoad(luaScript string) (string, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) LuaExists(sha1 ...string) (map[string]bool, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GeoAdd(key string, lonLatAddr ...interface{}) (int, error) {
...
func (rc *client) GeoGetPosList(key string, addr ...string) ([]*[2]float64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GeoGetPos(key string, addr string) (*[2]float64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GeoGetDistance(key string, addr1, addr2 string) (float64, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GeoGetRadius(key string, longitude, latitude, radius float64) ([]*position, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
...
func (rc *client) GeoGetRadiusByAddr(key string, address string, radius float64) ([]*position, error) {
	rdConn := rc.pool.Get()
	// 函数运行结束 ，把连接放回连接池
	defer rdConn.Close()
